  ...
  String convertTime(String time) {
    ...
    try {
      if (notificationSettingController.timeFormat == true) {
        // Convert 12-hour to 24-hour
        final DateTime dateTime = DateFormat("hh:mm a").parse(
            time); // 12-hour format
        String convertedTime = DateFormat("HH:mm ").format(
...
// set prayer times for upcoming prayers
  ...
  Future<void> fetchMissedPrayersCount() async {
    ...
    try {
      final requestUrl = 'http://182.156.200.177:8011/adhanapi/missed-prayers/?user_id=${userData
          .getUserData!.id}';
  ...
  DateTime parseTime(String time24) {
    ...
    if (match == null)
      return now; // Return current time if the format is invalid
  ...
  Future<void> fetchPrayerTime({DateTime? specificDate}) async {
    ...
    if (userData.getPrayerTimingsData.isEmpty) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          updateCalendarData = [];
          print(
              'Error fetching prayer times. Status Code: ${response
                  .statusCode}');
        }
      } catch (e) {
        print("kekej");
    ...
    if (getExtractedData.isNotEmpty) {
      ...
      String dhuhrTimeStr = convertTo12HourFormat(
          getExtractedData[0].timings?.dhuhr ?? 'N/A');
      ...
      islamicDate.value =
      '${hijriDate?.day ?? "Day"} ${hijriDate?.month?.en ?? "Month"} ${hijriDate
          ?.year ?? "Year"} ${hijriDate?.designation?.abbreviated ??
          "Abbreviation"}';
      print("baqar");
      print("isEmpty ${userData.getPrayerDurationForShia}");
      updatePrayerDuration =
      userData.getUserData!.fiqh.toString() == '0' // Shia fiqh
          ? {
        'Fajr': {
        ...
        'Dhuhr': {
          ...
          // Ends at sunset for Shia Muslims
        },
        'Asr': {
          'start': getExtractedData[0].timings?.dhuhr ?? 'N/A',
          // Allows Asr start after Dhuhr is prayed
          'end': getExtractedData[0].timings?.sunset ?? 'N/A'
        },
        'Maghrib': {
        ...
        'Isha': {
          'start': getExtractedData[0].timings?.maghrib ?? 'N/A',
          // Allows Isha start after Maghrib is prayed
          'end': '23:58 (IST)'
        },
      }
      ...
      // print(
      //     "getExtractedData[0].timings?.fajr ${getExtractedData[0].toJson()}");
      // updateUpcomingPrayerDuration =   userData.getUserData!.fiqh.toString() == '0' // Shia fiqh
      //     ? {
      //   'Fajr': {
      //     'start': getExtractedData[0].timings?.fajr ?? 'N/A',
      //     'end': getExtractedData[0].timings?.sunrise ?? 'N/A'
      ...
      if (userData.getUserData!.fiqh.toString() == '0') {
        ...
        if ((dhuhrPrayed || maghribPrayed) && (currentPrayer.value == 'Dhuhr' ||
            currentPrayer.value == 'Maghrib')) {
          print(
              "Dhuhr or Maghrib is prayed. Moving to the next prayer. ${currentPrayer
                  .value}");
          // moveToNextPrayer();
          if (currentPrayer.value == 'Dhuhr') {
            print("+++++++++++");
            prayerDuration['Dhuhr']!['end'] = DateFormat('HH:mm').format(
            ...
            if (maghribPrayed) {
              prayerDuration['Maghrib']!['end'] = DateFormat('HH:mm').format(
                  DateTime.now().subtract(Duration(minutes: 1)));
  ...
  String getCurrentPrayer(Map<String, Map<String, String>> c,
      String currentTime) {
    ...
    if (currentPrayer == 'Free') {
      nextPrayer.value =
          getNextPrayer(prayerDuration, currentTime); // Fetch next prayer
  ...
  String getNextPrayer(Map<String, Map<String, String>> prayerDuration,
      String currentTime) {
    String nextPrayer = '';
    ...
    for (var prayer in prayerDuration.keys) {
      ...
      if (currentTime.compareTo(prayerStartTime) < 0 ||
          currentTime.compareTo(prayerStartTime) == 0) {
        nextPrayer = prayer;
        print("NEXT PRAYER !@# $nextPrayer");
    ...
    if (nextPrayer.isEmpty) {
      ...
      if (currentTime.compareTo(ishaEndTime) >= 0 ||
          currentTime.compareTo(ishaEndTime) == 0) {
  ...
  void highlightCurrentPrayer() {
    prayerTimer = Timer.periodic(const Duration(minutes: 1), (timer) {
      ...
      DateTime start = DateTime(
          now.year, now.month, now.day, 5, 0); // Example start time
      DateTime end = DateTime(
          now.year, now.month, now.day, 18, 0); // Example end time
      if (now.isAfter(start) && now.isBefore(end)) {
        progressPercent.value = (now
            .difference(start)
            .inMinutes / end
            .difference(start)
            .inMinutes).clamp(0.0, 1.0);
      } else {
  ...
  void showNextPrayer() {
    ...
    if (currentPrayer.value == "Isha") {
      ...
      if (nextDayData.isNotEmpty) {
        ...
        upcomingPrayerStartTime.value =
            convertTo12HourFormat(nextDayFajrTiming);
        upcomingPrayerEndTime.value = convertTo12HourFormat(
            nextDayData['timings']['Sunrise']); // assuming Sunrise marks Fajr end time
      ...
      print("prayerDuration[nextPrayer.value] ${prayerDuration[nextPrayerName
          .value]!}");
      print("nextPrayerTimes['start'] ${nextPrayerTimes['start']!}");
      print("Next Prayer Name");
      print(nextPrayerName);
      upcomingPrayerStartTime.value =
          convertTo12HourFormat(nextPrayerTimes['start']!);
      upcomingPrayerEndTime.value =
          convertTo12HourFormat(nextPrayerTimes['end']!);
    }
  }
  ...
  String getNextPrayerName() {
    String currentTime = DateFormat('HH:mm').format(DateTime.now());
    String nextPrayerName = getNextPrayer(prayerDuration, currentTime);
  ...
  void _startCountdown(String fajrStartTime) {
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      if (upcomingRemainingTime.value.inSeconds <= 0) {
        ...
        upcomingRemainingTime.value -=
            Duration(seconds: 1); // Update remaining time
...
// Helper function to get tomorrow's date in the same format as the 'gregorian' date field
  ...
  void startRemainingTimeTimer() {
    ...
    if (currentPrayerStartTime.value.isEmpty ||
        currentPrayerEndTime.value.isEmpty) {
    ...
    remainingTimeTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (currentPrayerStartTime.value.isNotEmpty &&
          currentPrayerEndTime.value.isNotEmpty) {
        ...
        try {
          ...
          DateTime startTime = DateFormat('hh:mm a').parse(
              currentPrayerStartTime.value);
          DateTime endTime = DateFormat('hh:mm a').parse(
              currentPrayerEndTime.value);
          startTime = DateTime(
          ...
          if (now.isBefore(startTime)) {
            ...
            if (nextPrayerStartTime.value.isNotEmpty) {
              ...
              DateTime nextPrayerStart = DateFormat('hh:mm a').parse(
                  nextPrayerStartTime.value);
              nextPrayerStart = DateTime(
                  now.year, now.month, now.day, nextPrayerStart.hour,
                  nextPrayerStart.minute);
              ...
              if (gapDuration.isNegative) {
                print(
                    "Next prayer start time has passed, transitioning again.");
                timer.cancel();
                moveToNextPrayer();
              } else {
  ...
  void moveToNextPrayer() {
    ...
    String nextPrayerName = getNextPrayer(
        prayerDuration, DateFormat('HH:mm').format(DateTime.now()));
      ...
      if (currentPrayerStartTime.value.isNotEmpty &&
        ...
        if (now.isBefore(startTime)) {
          ...
          if (totalGapDuration == null) {
            totalGapDuration = startTime.difference(
                now); // gap duration from current time to start of next prayer
          }
          ...
          completionPercentage.value =
              1 - (gapDuration.inSeconds / totalGapDuration!.inSeconds);
          ...
          print("Current prayer has ended, moving to next prayer.");
          totalGapDuration =
          null; // Reset totalGapDuration for the next gap period
          moveToNextPrayer();
        } else {
          // We are within the current prayer, calculate progress as before
          totalGapDuration =
          null; // Reset totalGapDuration as it's no longer needed in a prayer period
...
//   }
  ...
  Future<void> getIsPrayed() async {
    try {
      ...
      Dialogs.showCustomBottomSheet(
        context: context, content: NoInternet(message: '$e', onRetry: get,),);
    }
    // if (response.statusCode == 200) {
  ...
  leaderboard() async {
    try {
      DateTime now = DateTime.now();
      String formattedDate = DateFormat('dd-MM-yyyy').format(now);
      String endpoint = 'prayer-response-friend/?user_id=${userData.getUserData!
  ...
  Future<void> markPrayerAsPrayed(String prayer) async {
    if (userData.getUserData!.fiqh.toString() ==
        '0') { // Only apply for Shia fiqh
      if (prayer == 'Dhuhr') {
...
// Sample usage for marking a prayer as prayed and saving updated times
  ...
  weeklyApi() async {
    try {
      String formattedDate = DateFormat('dd-MM-yyyy').format(
          selectedDate.value);
      String endpoint = 'friend-weekly-prayer-response/?user_id=${userData
          .getUserData!.id}&date=$formattedDate';
      final response = await ApiService().getRequest(endpoint);
      print("hju");
      print("Response Data: $response");
  ...
  bool isTimeWithinRange(String startTime, String endTime, String tapTime) {
    ...
    return (tapEventTime.isAfter(start) ||
        tapEventTime.isAtSameMomentAs(start)) &&
...
//     required BuildContext context,
//   }) async {
//     Get.back(); // Close any open dialogs or overlays
//     Dialogs.showLoading(context, message: 'Loading...');
//
//     DateTime date = DateTime.now();
//     String formattedDate = valDate ?? DateFormat('dd-MM-yyyy').format(date); // Updated to yyyy-MM-dd
//     print("formattedDate $formattedDate");
//
//     try {
//       // Parse dynamic `startTime` and `endTime` into `TimeOfDay`
//       TimeOfDay start = parseTimeOfDay(startTime ?? "13:00");
//       TimeOfDay end = parseTimeOfDay(endTime ?? "15:00");
//
//
//       // Current time as `TimeOfDay`
//       final now = DateTime.now();
//       final currentTime = TimeOfDay(hour: hour, minute: minute);
//       TimeOfDay  selected = parseTimeOfDay(currentTime.toString());
//       //final currentTime = TimeOfDay(hour: now.hour, minute: now.minute);
//
//       // Check if the current time is within the prayer time range
//       if (isTimeInRange(selected, start, end)==false) {
//         if (isFromMissed != true) {
//           // Hide loading dialog and show Snackbar
//           Get.snackbar(
//             'Out of Range',
//             'The current time is outside the prayer time range.',
//             backgroundColor: Colors.red,
//             colorText: Colors.white,
//             snackPosition: SnackPosition.BOTTOM,
//           );
...
//      // DateTime time = DateTime(0, 1, 1, currentTime.hour, currentTime.minute);
//      // String formattedTime = DateFormat('HH:mm').format(time);
//
//       DateTime time = DateTime(0, 1, 1, hour, minute);
//        String formattedTime = DateFormat('HH:mm').format(time);
...
//         fetchMissedPrayersCount();
//       } else {
//         throw Exception("Failed to submit prayer: ${response.statusCode}");
//       }
//     } catch (e) {
//       print('Error: $e');
//       Get.snackbar(
...
//       );
//     } finally {
//       Dialogs.hideLoading(); // Ensure loading dialog is hidden
//     }
//   }
//
...
//     required this.minute});}
  ...
  // String convertTimes(String time12Hour) {
  //   final DateTime dateTime = DateFormat("hh:mm a").parse(time12Hour);
  //   return DateFormat("HH:mm").format(dateTime);
  // }
  // //
  ...
  //     var request = http.Request(
  //       'POST',
  //       Uri.parse('http://182.156.200.177:8011/adhanapi/prayer-records/$formattedDate/'),
  //     );
  //
  //     request.body = json.encode({
  //       "user_id": int.parse(userId),
